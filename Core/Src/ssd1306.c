...
void ssd1306_InvertRectangle(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2) {
  ...
  for (uint8_t y = y1; y <= y2; y++) {
      for (uint8_t x = x1; x <= x2; x++) {
          // Get the pixel index in the buffer
          uint32_t buffer_index = x + (y / 8) * SSD1306_WIDTH;
          uint8_t pixel_mask = 1 << (y % 8);

          // Invert the pixel
          if (SSD1306_Buffer[buffer_index] & pixel_mask) {
              // If pixel is set (white), clear it (black)
              SSD1306_Buffer[buffer_index] &= ~pixel_mask;
          } else {
              // If pixel is clear (black), set it (white)
              SSD1306_Buffer[buffer_index] |= pixel_mask;
          }
      }
  }
